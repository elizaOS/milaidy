...
| MW-07 | P1 | API | API | MCP operations | ~~Missing end-to-end MCP config + runtime server-attach validation~~ Resolved: 37 e2e tests cover config CRUD (POST/GET/PUT/DELETE), server name guards, stdio/remote/env reject paths, prototype pollution guards ($include, constructor), PUT bulk validation, and runtime status endpoint behavior | Add integration test that writes MCP config, validates reject paths, and verifies status endpoint behavior | `bunx vitest run --config vitest.e2e.config.ts test/mcp-config.e2e.test.ts` | `test/mcp-config.e2e.test.ts` | Unsafe or broken MCP server execution |
