...
describe("agent admin routes", () => {
  ...
  test("unsafe reset path is rejected", async () => {
  ...
  test("reset clears only managed runtime env values", async () => {
    state.config = {
      env: {
    ...
    expect(process.env.CUSTOM_PLUGIN_KEY).toBe("custom-value");
