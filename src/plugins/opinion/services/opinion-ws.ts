...
export class OpinionWsService {
  ...
  private connect(apiKey: string) {
    try {
      ...
      this.ws.onmessage = (event) => {
        try { const data = JSON.parse(String(event.data)); this.handleMessage(data); } catch { /* ignore */ }
      };
      this.ws.onclose = () => {
      ...
      this.runtime?.logger?.warn?.(`Opinion WS: connection failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  private scheduleReconnect(apiKey: string) {
    ...
    this.runtime?.logger?.info?.(`Opinion WS: reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
    ...
    if (data.channel === "market.last.price" && data.data) {
      ...
      if (oldPrice !== undefined && oldPrice > 0) {
        ...
        if (change >= PRICE_ALERT_THRESHOLD) {
          ...
          this.runtime?.logger?.warn?.(`Opinion price alert: market ${data.data.marketId} moved ${direction} ${pct}%`);
        }
      }
