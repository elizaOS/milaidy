...
export const opinionContributor: AwarenessContributor = {
  ...
  async summary(_runtime: IAgentRuntime): Promise<string> {
    ...
    try {
      ...
      return summary.length <= SUMMARY_CHAR_LIMIT ? summary : summary.slice(0, SUMMARY_CHAR_LIMIT - 1) + "\u2026";
    } catch {
  ...
  async detail(_runtime: IAgentRuntime, level: "brief" | "full"): Promise<string> {
    ...
    try {
      ...
      if (!positions.length) { lines.push("No open positions."); return lines.join("\n"); }
      for (const p of positions) {
        const pnl = ((Number(p.currentPrice || 0) - Number(p.avgEntryPrice || 0)) * Number(p.shares || 0)).toFixed(2);
        const sign = Number(pnl) >= 0 ? "+" : "";
        lines.push(`- ${p.marketTitle}: ${(p.side ?? "").toUpperCase()} ${p.shares} @ ${p.avgEntryPrice} → ${p.currentPrice} (${sign}$${pnl})`);
      }
      if (level === "full") {
