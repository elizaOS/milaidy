...
export const checkOpinionPositionsAction: Action = {
  ...
  similes: ["OPINION_POSITIONS", "PREDICTION_PORTFOLIO", "MY_PREDICTIONS", "OPINION_HOLDINGS"],
  description: "Check current positions on Opinion.trade prediction markets. Shows each position's market, side, shares, average price, current price, and P&L.",
  ...
  handler: async () => {
    try {
      ...
      const lines = positions.map((p: any) => {
        const pnl = ((Number(p.currentPrice || 0) - Number(p.avgEntryPrice || 0)) * Number(p.shares || 0)).toFixed(2);
      ...
      return { text: `Failed to check positions: ${err instanceof Error ? err.message : String(err)}`, success: false };
    }
  },
  parameters: [],
};
