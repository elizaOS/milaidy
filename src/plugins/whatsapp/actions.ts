...
export const sendWhatsAppMessage: Action = {
  ...
  parameters: [
    {
      name: "phoneNumber",
      description:
        "The recipient's phone number in international format (e.g. +5522997585869). Include country code.",
      required: true,
      schema: { type: "string" },
    },
    {
  ...
  handler: async (
    ...
    if (cleanPhone.length < 8) {
      if (callback) {
        await callback({
          text: `The phone number "${phoneNumber}" doesn't look valid. Please include the country code (e.g. +5522997585869).`,
          actions: [],
        } as Content);
      }
      return { success: false };
