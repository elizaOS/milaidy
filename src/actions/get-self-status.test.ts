...
describe("GET_SELF_STATUS action", () => {
  ...
  it("validates successfully", async () => {
    const result = await getSelfStatusAction.validate({} as never, {} as never, {} as never);
  ...
  it("returns detail for a specific module", async () => {
    ...
    registry.register({
      id: "wallet", position: 30, trusted: true,
      summary: async () => "Wallet: test",
      detail: async (_rt, level) => level === "brief" ? "Wallet brief info" : "Wallet full info",
    });
    const rt = mockRuntime(registry);
    const result = await getSelfStatusAction.handler(
      rt, {} as never, {} as never,
  ...
  it("returns error when registry not available", async () => {
    const rt = { getService: vi.fn(() => null) } as unknown as Parameters<typeof getSelfStatusAction.handler>[0];
    const result = await getSelfStatusAction.handler(
      rt, {} as never, {} as never,
      { parameters: {} } as HandlerOptions,
    );
