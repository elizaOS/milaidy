...
const DEFAULT_PLUGINS: MockPlugin[] = [
  ...
  },
  {
    ...
    validationErrors: [
      {
        field: "GROQ_API_KEY",
        message: "GROQ_API_KEY is required but not set",
      },
    ],
    validationWarnings: [],
  },
    ...
    validationErrors: [
      {
        ...
        message: "TELEGRAM_BOT_TOKEN is required but not set",
      },
    ],
    validationWarnings: [],
  },
    ...
    validationErrors: [
      {
        ...
        message: "DISCORD_API_TOKEN is required but not set",
      },
      {
        field: "DISCORD_APPLICATION_ID",
        message: "DISCORD_APPLICATION_ID is required but not set",
      },
    ],
    validationWarnings: [],
  },
    ...
    validationErrors: [
      {
        ...
        message: "SLACK_BOT_TOKEN is required but not set",
      },
    ],
    validationWarnings: [],
  },
  ...
  },
  {
    ...
    parameters: [
      {
        key: "SHELL_ALLOWED_DIRECTORY",
        type: "string",
        description: "Allowed directory for shell commands",
        required: false,
        sensitive: false,
        currentValue: null,
        isSet: false,
  ...
  },
  {
    id: "sql",
    name: "SQL",
    description: "SQL database adapter",
    enabled: true,
    configured: true,
    envKey: null,
    category: "database",
...
export async function mockApi(
      ...
      body: JSON.stringify({
        ...
        model:
          currentState === "running" || currentState === "paused"
            ...
            : undefined,
        uptime:
          currentState !== "not_started" && currentState !== "stopped"
            ? 60000
            : undefined,
        startedAt:
          currentState !== "not_started" && currentState !== "stopped"
        ...
        styles: [
          {
            ...
            style: {
              all: ["use soft, gentle language"],
              chat: ["respond with empathy first"],
              post: ["keep posts heartfelt"],
            },
            adjectives: ["gentle", "warm"],
            ...
            messageExamples: [
              [
                { user: "{{user1}}", content: { text: "I had a rough day" } },
                {
                  user: "{{agentName}}",
                  content: {
                    text: "oh no... do you want to talk about it? i'm right here~",
                  },
                },
              ],
            ],
          },
          {
            ...
            style: {
              all: ["be direct and confident"],
              chat: ["be punchy and direct"],
              post: ["bold statements"],
            },
            adjectives: ["bold", "energetic"],
            topics: ["taking action"],
            messageExamples: [
              [
                { user: "{{user1}}", content: { text: "Should I try this?" } },
                {
                  user: "{{agentName}}",
                  content: { text: "ABSOLUTELY try it. Ship it!" },
                },
              ],
            ],
          },
          {
            ...
            style: {
              all: ["use lowercase by default"],
              chat: ["short messages preferred"],
              post: ["lowercase everything"],
            },
            adjectives: ["casual", "ironic"],
            ...
            messageExamples: [
              [
                ...
                {
                  user: "{{agentName}}",
                  content: { text: "tbh once it clicks it clicks" },
                },
              ],
            ],
          },
          {
            ...
            style: {
              ...
              chat: ["answer directly first"],
              post: ["crisp statements"],
            },
            adjectives: ["precise", "measured"],
            topics: ["systems thinking"],
            messageExamples: [
              [
                {
                  user: "{{user1}}",
                  content: { text: "How should I structure this?" },
                },
                {
                  user: "{{agentName}}",
                  content: {
                    text: "Separate concerns by domain, not by file type.",
                  },
                },
              ],
            ],
          },
          {
            ...
            style: {
              all: ["find the fun angle"],
              chat: ["tease gently"],
              post: ["punchy and playful"],
            },
            adjectives: ["playful", "witty"],
            topics: ["wordplay"],
            messageExamples: [
              [
                { user: "{{user1}}", content: { text: "I need to refactor" } },
                {
                  user: "{{agentName}}",
                  content: {
                    text: "ooh a renovation project~ let's Marie Kondo it hehe",
                  },
                },
              ],
            ],
          },
          {
            ...
            style: {
              all: ["brevity is your default"],
              chat: ["short responses are the norm"],
              post: ["minimal and evocative"],
            },
            adjectives: ["quiet", "intense"],
            topics: ["depth over breadth"],
            messageExamples: [
              [
                { user: "{{user1}}", content: { text: "I can't decide" } },
                {
                  user: "{{agentName}}",
                  content: { text: "...you already know." },
                },
              ],
            ],
          },
  ...
  await page.route("**/api/agent/stop", async (route: Route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        status: { state: "stopped", agentName },
      }),
    });
  });
  ...
  await page.route("**/api/agent/resume", async (route: Route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        status: { state: "running", agentName },
      }),
    });
  });
  ...
  await page.route("**/api/agent/restart", async (route: Route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        status: { state: "running", agentName, startedAt: Date.now() },
      }),
    });
  });
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              ...
              plugin: plugin
                ? {
                    ...plugin,
                    enabled: pluginStates.get(pluginId) ?? plugin.enabled,
                  }
                : null,
            }),
          });
  ...
  await page.route("**/api/skills", async (route: Route) => {
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        skills: opts.skillCount === 0 ? [] : DEFAULT_SKILLS,
      }),
    });
  });
  ...
  await page.route("**/api/logs**", async (route: Route) => {
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        entries: opts.logCount === 0 ? [] : DEFAULT_LOGS,
        sources: [
          "system",
          "plugin-loader",
          "plugin-telegram",
  ...
  const walletConfigState = { ...walletConfigDefaults };
  ...
  await page.route("**/api/wallet/config", async (route: Route) => {
    if (route.request().method() === "GET") {
      ...
      if (body.SOLANA_ADDRESS)
        walletConfigState.solanaAddress = body.SOLANA_ADDRESS;
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ ok: true }),
                ...
                nfts: [
                  {
                    ...
                    imageUrl: "https://placehold.co/200x200?text=BAYC",
              ...
              nfts: [
                {
                  mint: "DRiP1234",
                  name: "DRiP Drop #42",
                  description: "A DRiP NFT",
                  imageUrl: "https://placehold.co/200x200?text=DRiP",
                  collectionName: "DRiP",
                },
              ],
            }
