...
Your agent can now reason and act on its own between conversations. When autonomous mode is enabled, the agent runs a continuous loop — observing its environment, making decisions, and executing actions without waiting for your input. Use it for background monitoring, proactive messaging, or any workflow where the agent should act independently. Toggle it from the dashboard or via the REST API. See the [autonomous mode guide](/guides/autonomous-mode) for details.

## Triggers and scheduled tasks

You can now schedule tasks that wake your agent at defined times or intervals. Three trigger types are available: recurring intervals, one-time future tasks, and cron expressions. Create triggers from the dashboard, the REST API, or by asking your agent in chat — it extracts the schedule from natural language. Triggers survive restarts and include built-in health metrics. See the [triggers guide](/guides/triggers) for details.

## Talk mode

Full voice conversations are now available on the desktop app. Talk mode combines offline speech-to-text via Whisper.cpp, streaming text-to-speech via ElevenLabs (with system TTS fallback), and voice activity detection into a hands-free loop — you speak, the agent listens, thinks, and responds aloud. Configure Whisper model size, VAD sensitivity, and TTS provider from [dashboard settings](/apps/dashboard/talk-mode).

## Local model support

Run AI models entirely on your machine with no cloud dependency. Milady can download and cache models from HuggingFace or pull them via Ollama for vision, text generation, TTS, speech-to-text, and embeddings. Choose from models like Llama 3.2, Whisper, BLIP, Parler TTS, and MiniLM — managed through the CLI or the REST API. See the [local models guide](/guides/local-models) for details.

## Chrome extension for browser control

The Milady Browser Relay Chrome extension lets your agent control browser tabs via the Chrome DevTools Protocol. Your agent can read page content, execute JavaScript, navigate URLs, capture screenshots, and observe network requests — all in real time over a WebSocket connection. Install from source with the CLI or manually. See the [Chrome extension guide](/apps/chrome-extension) for details.

## Mobile app (iOS and Android)

The full Milady dashboard is now available on iOS and Android. The Capacitor-based app runs inside a native WebView with access to device hardware — camera, microphone, haptics, and push notifications. Available through the App Store, Google Play, and TestFlight. See the [mobile app guide](/apps/mobile) for details.

## Updates

- **Step-by-step tutorials** — Three new tutorials walk you through building a [Discord bot](/guides/tutorial-discord-bot), a [Telegram bot](/guides/tutorial-telegram-bot), and an [autonomous agent](/guides/tutorial-autonomous-agent) from scratch.
- **Learning paths** — A new [learning paths](/guides/learning-paths) page organizes guides by role and experience level, so you can find the right starting point whether you're a first-time user, a plugin developer, or deploying to production.
- **Plugin development docs** — Comprehensive guides now cover the full plugin lifecycle: [architecture](/plugins/architecture), [creating a plugin](/plugins/create-a-plugin), [development](/plugins/development), [testing](/plugins/testing), [patterns](/plugins/patterns), and [publishing](/plugins/publish).
</Update>

<Update label="2026-05-14" description="Week of May 8–14" tags={["Feature", "Update"]}>
## Sandbox live monitor

The [sandbox](/guides/sandbox) now includes a live monitor panel alongside the interactive terminal. A noVNC desktop viewer lets you watch your sandboxed agent interact with a virtual desktop in real time — including browser activity, window management, and screen previews. Connection status and browser endpoints are visible at a glance so you always know what the sandbox is doing.

## Custom chat commands

You can now select text in a [chat](/dashboard/chat) conversation and save it as a reusable `/command`. Give it a name and invoke it later by typing the command prefix. This makes it easy to turn frequently used prompts or instructions into one-tap shortcuts.

## Provider switcher

A new provider management panel lets you browse all installed AI provider plugins, check configuration status, and switch between providers — including [Eliza Cloud](/guides/cloud), local models, and subscription-based access — without restarting your agent. See the [model providers](/model-providers) guide for details.

## Runtime inspector

An advanced runtime debug view is now available in the dashboard. It displays a full snapshot of your agent's internal state split into tabbed sections — actions, providers, plugins, services, and evaluators — with explicit load order metadata. Useful for diagnosing why a plugin isn't loading or which actions are registered.

## Status bar

A compact status bar at the top of the dashboard shows your active AI provider icons (with per-provider dropdowns), cloud credit balance, and wallet summary. This gives you at-a-glance awareness of your agent's current configuration without opening Settings.

## Onboarding contextual help

Each step of the onboarding wizard now includes contextual help panels that explain what the setting does, when to use it, what happens if you skip it, and the recommended default. New users can make informed decisions about provider selection, RPC configuration, permissions, and voice settings during first-run setup.

## Three additional UI languages

The app interface is now available in Korean, Portuguese, and Spanish, in addition to the existing English and Simplified Chinese. Language selection applies across the full dashboard and companion view.

## Updates

- **Connector health monitoring** — A background health monitor now checks whether your configured connectors (Discord, Telegram, Twitter, Slack, Farcaster) still have their corresponding plugin loaded. If a connector goes down, a warning banner appears automatically via WebSocket so you can act before users notice.
- **Credit exhaustion detection** — When a cloud provider returns a billing error (HTTP 402 or 429 with quota context), the dashboard now identifies the specific provider and shows targeted guidance instead of a generic failure message.
- **Twitter and NFT verification** — Two new verification methods are available for whitelist eligibility: post a verification tweet containing your agent name and wallet address, or prove ownership of an OG Milady Maker NFT. Verified addresses feed into a Merkle tree for on-chain whitelist proofs. See the [wallet guide](/guides/wallet) for details.
</Update>
