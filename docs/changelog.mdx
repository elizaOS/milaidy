...
The Windows download is now a proper one-click installer built with Inno Setup. The previous download required a separate payload archive to run — this one installs and launches on its own. MSIX packaging remains available as a secondary option. See the [installation guide](/installation) for details.

## Intel Mac desktop support

The macOS DMG for Intel Macs now ships correctly. Previous releases could fail to launch because the packaged native binaries were built for Apple Silicon only. Both Intel (x64) and Apple Silicon (arm64) DMGs are now published with every release.

## Desktop startup resilience

If the agent runtime fails to load — for example, due to a missing native module — the desktop app now keeps the API server running and shows a clear error message instead of a blank screen. You can retry loading from the UI without restarting the app.

## Bug fixes

- Safe avatar index validation prevents display errors when `STREAM_AVATAR_INDEX` is out of range
- Oversized voice API requests are now properly rejected with a 413 error and clear size limits
</Update>
