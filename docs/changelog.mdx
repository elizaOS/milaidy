...
## Expanded platform connectors

Milady now supports **17 messaging and social platforms** out of the box. In addition to Discord, Telegram, Twitter, Slack, and WhatsApp, you can now connect your agent to:

- **Farcaster** — cast, reply, and react on the decentralized social protocol via Neynar
- **Lens Protocol** — post and interact on the Lens V3 social graph
- **Signal** — private and group messaging via signal-cli
- **Mattermost** — self-hosted team chat with bot token auth
- **Microsoft Teams** — Azure Bot integration with threaded replies
- **Google Chat** — service account integration with webhook delivery
- **Matrix**, **Nostr**, and **Feishu** — additional protocol support

Each connector auto-enables when you add its credentials to your config. See the [connectors overview](/guides/connectors) and individual setup guides in the [plugin registry](/plugin-registry/platform/discord) for details.

## Computer use plugin

Your agent can now control the desktop — taking screenshots, moving the mouse, clicking, and typing. The [computer use plugin](/plugin-registry/computeruse) lets agents interact with any GUI application on the host machine, unlocking automation beyond API-only workflows. Install with `milady plugins install computeruse`.

## Agent export and import

You can now export your entire agent — character, memories, knowledge, relationships — as an encrypted, password-protected portable archive. Move agents between machines or share them with others. Import restores the full state with automatic ID remapping to avoid collisions. See the [agent export guide](/guides/agent-export) for details.

## Trajectories viewer

A new [Trajectories](/advanced/trajectories) tab in the dashboard gives you full visibility into your agent's LLM call history. Inspect prompts, responses, token usage, and latency for every model call your agent makes.

## Config includes

The [`$include` directive](/guides/config-includes) lets you split agent configuration across multiple files and compose them together. Share base configs across agents, separate environment-specific settings, and keep large configurations organized.

## Triggers and scheduled tasks

[Triggers](/guides/triggers) let you schedule tasks that wake your agent on intervals, at specific times, or via cron expressions. Combined with the always-loaded [cron plugin](/plugin-registry/cron), you can set up recurring automations like scheduled posts, periodic monitoring, and batch processing — all declaratively in your config.

## Updates

- **Farcaster connector hardened** — A comprehensive test suite (56 tests) now covers the Farcaster connector end-to-end, including cast publishing, reactions, channel posting, media attachments, and error handling.
- **Lens Protocol connector hardened** — 51 new tests validate the Lens V3 connector across authentication, posting, reactions, notifications, and media workflows.
- **Configuration templates** — Eight ready-to-use [configuration templates](/guides/config-templates) are now available for common deployment scenarios, from minimal single-provider setups to multi-connector production deployments.
</Update>
