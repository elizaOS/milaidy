...
The skills view now includes a marketplace where you can browse, search, and install skills directly from the dashboard. A built-in security review system scans installed skills and flags issues — quarantined skills are visually marked, and blocked skills cannot be enabled until reviewed. See the [skills guide](/plugins/skills) for details.

## Five new platform connectors

Your agent can now connect to five additional platforms:

- **Feishu / Lark** — enterprise messaging with bot, chat, and workflow notification support.
- **Twilio** — voice calls and SMS messaging.
- **Twitch** — live chat integration for streaming.
...
- **Voice wake word** — You can now configure a wake word (default: "milady") so your agent listens for a trigger phrase before responding. Adjust sensitivity and model settings from the voice settings panel.
...
- **Dashboard themes** — Six visual themes are now available, controlling colors, typography, borders, and animations across the entire dashboard. See the [themes guide](/guides/themes).
- **Built-in VRM avatars** — Eight 3D VRM avatars with 29 emotes across six categories are now available out of the box. See the [emotes guide](/guides/emotes).
</Update>
