...
The mobile app uses 9 custom Milady Capacitor plugins, each providing native capabilities with web fallbacks:
...
1. **Discovery** (native only) — the app scans the local network for Milady gateways.
...
- `plugins` — access to all Milady plugins with fallback support.
