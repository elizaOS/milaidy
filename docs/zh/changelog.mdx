...
<Update label="2026-05-14" description="5月8日至14日当周" tags={["Feature", "Update"]}>
## Chrome 扩展程序实现浏览器自动化

您的代理现在可以通过 Milady Browser Relay Chrome 扩展程序直接控制浏览器标签页。该扩展程序将 Chrome DevTools 协议与代理运行时连接起来，支持页面检查、JavaScript 执行、导航、截图捕获和自主浏览工作流——只需在工具栏上点击一下即可切换。通过 `milady browser extension install` 从源代码安装。详见 [Chrome 扩展程序指南](/apps/chrome-extension)。

## 自主模式

代理现在可以在对话之间独立推理和行动。启用后，一个持续运行的循环会观察环境、做出决策并执行操作，无需等待用户输入——适用于后台监控、定时工作流和主动行为。可从仪表板或通过 `GET /api/agent/autonomy` 端点切换。详见[自主模式指南](/guides/autonomous-mode)。

## 事件钩子

您现在可以通过事件驱动的处理器来扩展代理，响应命令、会话、代理生命周期和网关事件。在钩子目录中放置一个 `HOOK.md` 文件，定义处理器，代理将在启动时加载它。钩子允许在会话创建、代理启动或用户命令时执行自定义逻辑——无需编写完整插件。详见[钩子指南](/guides/hooks)。

## 技能市场

技能——基于 Markdown 的扩展，教代理如何执行特定任务——现在可以从仪表板浏览、搜索、安装和管理。每个技能是一个包含指令的文件夹，在运行时注入代理上下文。您也可以编写自己的技能。详见[技能指南](/plugins/skills)。

## 媒体生成

代理现在可以生成图像、视频和音频，或使用 AI 视觉分析图像——全部通过内置操作完成。支持的提供商包括 FAL、OpenAI、Google、xAI 和 Eliza Cloud（无需 API 密钥）。让代理"画一个日落"或"描述这张图片"，它会自动选择提供商。详见[媒体生成指南](/guides/media-generation)。

## 触发器和定时任务

您现在可以设定在指定间隔、特定时间或通过 cron 表达式唤醒代理的任务。可通过仪表板或 REST API 创建三种类型的触发器：`interval` 用于循环任务，`once` 用于一次性未来任务，`cron` 用于 cron 定时操作。详见[触发器指南](/guides/triggers)。

## 自定义操作编辑器

直接从仪表板定义新的代理功能，无需编写插件代码。自定义操作支持 HTTP、shell 和代码处理器。描述操作、设置参数，代理即可在对话中调用。详见[自定义操作指南](/guides/custom-actions)。

## 插件弹出系统

您现在可以将任何插件的源代码克隆到本地、修改它，并让运行时加载您的本地副本而非 npm 包。弹出系统支持与上游同步、通过 pull request 贡献更改，以及弹出核心包进行深度定制。详见[插件弹出指南](/plugins/plugin-eject)。

## 移动应用

Milady 现已通过基于 Capacitor 的原生应用登陆 iOS 和 Android。完整的仪表板体验在原生 WebView 中运行，可访问设备硬件——摄像头、触觉反馈、推送通知等。详见[移动应用指南](/apps/mobile)了解设置和构建说明。

## 更新

- **主题和头像自定义** — 六个内置视觉主题和八个 3D VRM 头像（含 29 个表情）现可在设置中配置。即时切换主题，拖放自定义 VRM 头像。详见[主题指南](/guides/themes)。
- **配置包含** — 使用 `$include` 指令将代理配置拆分为多个文件。为不同环境组合模块化配置，或在代理之间共享基础配置。详见[配置包含指南](/guides/config-includes)。
- **本地模型支持** — 通过 Ollama 在本地运行模型，无需将数据发送到外部提供商。详见[本地模型指南](/guides/local-models)。
- **表情动画** — 代理可以通过六个类别的 30 个 VRM 表情动画表达情绪，在对话中自动触发或从表情选择器手动触发。详见[表情指南](/guides/emotes)。
</Update>
...
代理现在可以在隔离的容器沙箱内运行代码、浏览网页、截取屏幕截图、录制音频并与桌面应用程序交互。沙箱支持可配置的安全模式、网络策略、审计日志以及远程签名服务——该服务将钱包密钥保存在主机上，同时沙箱中的代理提交交易。详情请参阅[沙箱指南](/zh/guides/sandbox)。
