...
2. Generates a random gateway token (via `openssl rand -hex 32`)
...
# Bind to 0.0.0.0 in containers so the service is reachable
...
<Info>
The docker-compose.yml passes `CLAUDE_AI_SESSION_KEY`, `CLAUDE_WEB_SESSION_KEY`, and `CLAUDE_WEB_COOKIE` through to both services. Add additional provider keys by extending the `environment` section.
</Info>
