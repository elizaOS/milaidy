...
Milady is a cross-platform AI agent runtime built on top of [ElizaOS](https://elizaos.ai). It delivers the same backend across CLI, web, desktop (Electron), and mobile (Capacitor) surfaces through a layered architecture.
...
|  Tabs: Chat, Character, Knowledge, Connectors, Apps,          |
|         Wallets, Advanced (Plugins, Skills, Triggers, DB, ...) |
+----------------------------+----------------------------------+
                             | HTTP + WebSocket
                             v
+---------------------------------------------------------------+
...
|  Commands: start, update, status, channel, ...                |
...
The CLI entry point (`src/entry.ts`) is the bootstrap for all Milady operations.

- Sets process title to `milady`
...
The runtime (`src/runtime/eliza.ts`) starts an `AgentRuntime` from `@elizaos/core` with Milady's plugin configuration.

Key responsibilities:
- Loads and resolves core + optional + user-installed plugins
- Creates the Milady plugin (workspace context, session keys, lifecycle actions)
...
- Token-based authentication when `MILADY_API_TOKEN` is set
...
<CardGroup cols={2}>
  ...
  <Card title="Connectors View" icon="plug">
    Configure Discord, Telegram, Slack, and other messaging connectors
  </Card>
  <Card title="Apps View" icon="grid">
    Browse, install, and manage ElizaOS apps from the marketplace
  </Card>
  <Card title="Inventory View" icon="wallet">
...
|----------|----------|-------|
| CLI | `milady start` | Direct Node.js process |
...
Milady assembles its capabilities through ElizaOS's plugin system. Plugins are loaded in two tiers.
...
The `milady` plugin (`src/runtime/milady-plugin.ts`) provides:
...
Milady uses a Zod-validated JSON configuration file (`milady.json`) stored in the state directory (`~/.milady/`).
