...
Milady provides CLI commands to manage configuration without editing JSON directly:
...
# Set a config value
milady config set gateway.port 19000
milady config set agents.defaults.thinkingDefault "high"
```

<Tip>
Use `milady configure` for initial setup. It walks through model provider selection, agent personality, and connector setup interactively.
</Tip>
