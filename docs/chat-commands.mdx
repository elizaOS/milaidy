...
Milaidy supports several layers of in-session commands: TUI slash commands for the terminal interface, runtime commands controlled by configuration, custom command handlers via the hooks system, and plugin-provided commands.
...
The web Control UI (opened via `milaidy dashboard`) provides its own command interface through the API server. Commands sent through the web chat are processed by the same ElizaOS runtime, so all runtime commands, hook handlers, and plugin-provided actions are available.
