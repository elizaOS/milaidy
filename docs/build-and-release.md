...
- **macOS notarization is now centralized in the staged artifact flow** — the `Build Electrobun app` step still performs the platform signing pass, but it sets `MILADY_ELECTROBUN_NOTARIZE=0` so notarization happens only inside `apps/app/electrobun/scripts/stage-macos-release-artifacts.sh`. **Why:** the Intel release failure was a transient `notarytool` network error inside Electrobun’s internal notarization poll. The staged `.app` flow already owns retries and produces the real `.app.zip`/`.dmg` release artifacts, so it is the right single source of truth.
- **Actions artifacts are now explicitly CI transport bundles** — the workflow uploads them as `electrobun-*-ci-bundle` and includes `CI-ARTIFACT-README.txt` in the artifact root. **Why:** downloading from the Actions run page always produces an outer GitHub artifact zip, which made `*.app.zip` look like a broken zip-inside-zip installer. The actual end-user installers remain the GitHub Release assets.
