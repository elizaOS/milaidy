...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/agent/self-status` | Get a capabilities and self-awareness snapshot (wallet, plugins, permissions, trade readiness) |
...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/api/wallet/trade/preflight` | Check BSC trade readiness (wallet, RPC, chain, gas) |
| `POST` | `/api/wallet/trade/quote` | Get a BSC trade quote with routing |
| `POST` | `/api/wallet/trade/execute` | Execute or prepare a BSC trade (unsigned or local-key) |
| `GET` | `/api/wallet/trade/tx-status` | Check on-chain status of a BSC transaction. Param: `hash` |
| `GET` | `/api/wallet/trading/profile` | Get trading P&L profile from local ledger. Params: `window`, `source` |
| `POST` | `/api/wallet/transfer/execute` | Execute or prepare a BNB/ERC-20 token transfer |
| `POST` | `/api/wallet/production-defaults` | Apply production wallet configuration defaults |

### Privy Wallet

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/privy/status` | Check Privy wallet provisioning status |
| `POST` | `/api/privy/login` | Start Privy wallet login flow |
| `POST` | `/api/privy/logout` | Log out of Privy wallet session |
...
| `GET` | `/api/stream/source` | Get active stream capture source |
| `POST` | `/api/stream/source` | Set active stream capture source |
