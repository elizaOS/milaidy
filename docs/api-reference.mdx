...
The Milaidy API server runs on port **2138** by default. All endpoints are prefixed with `/api/` unless otherwise noted. Authentication is required when `MILAIDY_API_TOKEN` is set.
...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/config` | Get the current milaidy.json configuration |
