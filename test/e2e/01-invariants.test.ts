...
describe("Tier 1: System Invariants", () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      miladyProcess.on("close", (code) => {
        ...
        if (!portBound) {
          reject(
            new Error(
              `Process exited early with code ${code}. Output: ${stderrOutput}`,
            ),
          );
        }
      });
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `Timeout waiting for port binding. Output: ${combinedOutput}`,
          ),
        );
      }, TEST_TIMEOUT - 5000);
    });
  ...
  test("INV-NATIVE-01: Common native dependency crashes do not appear in logs", () => {
    ...
    expect(
      outputLower.includes("cannot find module '@elizaos/plugin-pi-ai'"),
    ).toBeFalse();
